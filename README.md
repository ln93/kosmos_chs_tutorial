 谨防虚假游戏，务必备份系统！
  ...
  本文基于KosmosV13编写，该版本基于Atmosphere0.9.2整合，使用hekate注入系统，支持7.0-8.1系统。
...
建议你找到”Latest release”图标，然后在右边找到Assets下方的Kosmos-V13.zip并下载。

 如果你希望玩破解游戏，你需要自行添加ES Patch，因此你还需要在同一地方一并下载ES.patches.7z。


点击Assets即可展开下载链接。下载前两项即可。本图中的版本号可能与实际不符 
    如果你不安装es.patches，那么运行破解游戏时必然会报错。

TegraRCMGUI
    TegraRcmGUI

为了启动破解系统，你需要在这里下载最新版本的TegraRcmGUI_v2.5_portable.zip
...
耐心等待备份完成，最后点击右上角close即可。
    ...
    然后，建议你现在立刻制作一个虚拟系统。这对你的系统安全很有帮助。你可以参考：
...
选择Browse SD card，找到你的nsp游戏安装文件


选择Browse SD card，之后用方向键和A键找到并选择你的nsp游戏安装文件。


选择你的游戏安装目的地——一般选择SD card。
...
玩游戏
    建议你使用本地账户玩游戏，不要使用在线账户。一些长期联网的骨灰级破解玩家表示，使用本地账户玩nsp游戏可以有效降低被ban的概率。
...
一些进阶内容：
    ...
    改变Switch主题布局（Layout）(使用WEXOS时，win10用户需要手动配置报错的DLL的权限。)
